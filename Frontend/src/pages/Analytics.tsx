...
//   // Date Formatting Helper (Mar 4, 2026, 10:16 PM)
...
//     if (activeCategory === "Total Analysis") return dummySignals;
//     if (activeCategory === "Trades Won") return dummySignals.filter(s => s.accuracy_status === "Won");
//     if (activeCategory === "Trades Lost") return dummySignals.filter(s => s.accuracy_status === "Loss");
//     if (activeCategory === "Pending Trades") return dummySignals.filter(s => s.accuracy_status === "Pending");
//     return [];
//   }, [activeCategory]);
...
//           <p className="text-xs font-bold text-muted-foreground tracking-widest uppercase italic opacity-60">Deep Performance Metrics</p>
//         </div>
//       </div>
...
//         <div className="lg:col-span-1 bg-[#0d0d0d] border border-white/5 p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden">
...
//               <RechartsPie>
//                 <Pie data={[{ name: "Won", value: stats.won, color: "#00ff88" }, { name: "Loss", value: stats.loss, color: "#ff4444" }]} innerRadius={75} outerRadius={95} paddingAngle={10} dataKey="value" stroke="none">
...
//           <div className="h-72">
//             <ResponsiveContainer width="100%" height="100%">
//               <AreaChart data={dummySignals.slice().reverse()}>
...
//       <div className="bg-[#0d0d0d] border border-white/5 p-8 rounded-[3rem] shadow-2xl overflow-hidden relative group">
//           <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4 relative z-10">
//             <div>
//               <h3 className="text-sm font-black uppercase tracking-[0.3em] text-white flex items-center gap-3"><BrainCircuit size={18} className="text-primary" /> AI Module Performance Breakdown</h3>
//               <p className="text-[10px] text-muted-foreground font-bold uppercase mt-1 tracking-widest">System Efficiency Audit</p>
//             </div>
//           </div>
//           <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8 relative z-10">
//             {[
//               { name: 'FUSION ENGINE', score: 89, color: 'text-primary', bar: 'bg-primary', status: 'Optimal' },
//               { name: 'TECHNICAL ANALYZER', score: 74, color: 'text-neon-amber', bar: 'bg-neon-amber', status: 'Stable' },
//               { name: 'SENTIMENT CORE', score: 82, color: 'text-blue-400', bar: 'bg-blue-400', status: 'High' },
//               { name: 'ENSEMBLE UNIT', score: 95, color: 'text-neon-green', bar: 'bg-neon-green', status: 'Maximum' }
//             ].map((module, idx) => (
//               <div key={idx} className="space-y-3">
//                 <div className="flex justify-between items-end">
//                   <div className="space-y-1"><span className="text-[10px] font-black text-muted-foreground tracking-widest uppercase">{module.name}</span><div className={`text-[10px] font-bold ${module.color} flex items-center gap-2 uppercase`}><Zap size={10} className="fill-current" /> {module.status}</div></div>
//                   <span className="text-lg font-black text-white italic tracking-tighter">{module.score}%</span>
//                 </div>
//                 <div className="relative h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
//                   <motion.div initial={{ width: 0 }} animate={{ width: `${module.score}%` }} transition={{ duration: 2, delay: idx * 0.2 }} className={`absolute top-0 left-0 h-full rounded-full ${module.bar}`} />
//                 </div>
//               </div>
//             ))}
//           </div>
...
//       {/* Detail Registry Modal */}
//       <AnimatePresence>
//         {activeCategory && (
//           <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/90 backdrop-blur-md" onClick={() => setActiveCategory(null)}>
//             <motion.div 
...
//                 <div><h2 className="text-2xl font-black text-white uppercase italic tracking-tight">{activeCategory}</h2><p className="text-[10px] text-muted-foreground font-bold tracking-widest uppercase mt-1">Audit log of system predictions</p></div>
...
//                   <div key={trade.id} className="bg-white/5 border border-white/5 p-6 rounded-[2rem] hover:border-white/10 transition-all group">
//                     <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-center">
//                       <div className="space-y-1"><span className="text-[9px] font-black text-muted-foreground uppercase tracking-widest">Prediction Unit</span><div className="text-sm font-bold text-white flex items-center gap-2"><div className={`w-1.5 h-1.5 rounded-full ${trade.signal === 'BUY' ? 'bg-neon-green' : 'bg-neon-red'}`} />{trade.module} <span className="text-[10px] opacity-40">#{trade.id}</span></div></div>
//                       <div className="space-y-1"><span className="text-[9px] font-black text-muted-foreground uppercase tracking-widest">Entry Detail</span><p className="text-xs font-mono text-white font-bold tracking-tighter">${trade.trade_entry.toLocaleString()}</p><p className="text-[10px] text-muted-foreground font-bold uppercase tracking-tighter">{formatDateTimeStandard(trade.created_at)}</p></div>
//                       <div className="space-y-1"><span className="text-[9px] font-black text-muted-foreground uppercase tracking-widest">Exit Detail</span><p className={`text-xs font-mono font-bold tracking-tighter ${trade.accuracy_status === 'Won' ? 'text-neon-green' : trade.accuracy_status === 'Loss' ? 'text-neon-red' : 'text-blue-400'}`}>{trade.accuracy_status === 'Pending' ? `TARGET: $${trade.trade_tp}` : `EXIT: $${trade.exit_price}`}</p><p className="text-[10px] text-muted-foreground font-bold uppercase tracking-tighter">{trade.exit_time ? formatDateTimeStandard(trade.exit_time) : "Position Active"}</p></div>
